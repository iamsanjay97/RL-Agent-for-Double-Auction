...
public class MarketManagerService implements MarketManager, Initializable, Activatable {
  ...
  private final Boolean isTraining = true;
  ...
  public Double targetQ = 175.0;
  public Double multiplier = 2.25;
  ...
  public synchronized void activate (int timeslotIndex)
  {
    ...
    for (int proximity = 1; proximity <= 24; proximity++)
    {
      ...
      // neededMWh[proximity-1] = messageManager.collectNetDemand(deliveryTimeslot) / 1000.0;
      neededMWh[proximity-1] = targetQ;
  ...
  private void submitOrderDDPG(double[] neededMWh)
  {
    ...
    // for(int proximity = 22; proximity <= 24; proximity++)
    {
      ...
      Pair<Double, Double> action = getDDPGAction(proximity, timeslot, neededMWh[proximity-1]);        // Random action for training 

      Double avgBuyBalancingPrice = wholesaleMarketInformation.getAvgMCP(timeslot)*multiplier; // balancingMarketInformation.getAvgBuyBalancingPrice(); // (for artificial demand)
      ...
      {    
        ...
        System.out.println("Actions: " + l1 + " :: " + l2 + ", Balancing Price: " + avgBuyBalancingPrice);
  ...
  private Pair<Double, Double> getDDPGAction(Integer proximity, Integer timeslot, double neededMWh)
  {
    marketTransactionInformation = messageManager.getMarketTransactionInformation();
    wholesaleMarketInformation = messageManager.getWholesaleMarketInformation();
  ...
  private void calculateReward(Integer timeslotIndex, double[] neededMWh)
  {
    ...
    Double avgBuyBalancingPrice = wholesaleMarketInformation.getAvgMCP(timeslotIndex)*multiplier; //balancingMarketInformation.getAvgBuyBalancingPrice(); // (for artificial demand)
